	...
	for r in test_data:
		...
		for x in range(0, len(r)):
			if r[x] == Yes:
				...
				isR = isR * ( (trainedR[x][0]+1) / (trainedR[x][2]+1) )
				isRlog = isRlog + math.log( (trainedR[x][0]+1) / (trainedR[x][2]+1) )
				# print("D=Yes", (trainedD[x][0]+1), "divide by", (trainedD[x][2]+1))
				isD = isD * ( (trainedD[x][0]+1) / (trainedD[x][2]+1) )
				isDlog = isDlog + math.log( (trainedD[x][0]+1) / (trainedD[x][2]+1) )
			if r[x] == No:
				# print("R=Nay", (trainedR[x][1]+1), "divide by", (trainedR[x][2]+1))
				isR = isR * ( (trainedR[x][1]+1) / (trainedR[x][2]+1) )
				isRlog = isRlog + math.log( (trainedR[x][1]+1) / (trainedR[x][2]+1) )
				# print("D=Nay", (trainedD[x][1]+1), "divide by", (trainedD[x][2]+1))
				isD = isD * ( (trainedD[x][1]+1) / (trainedD[x][2]+1) )
				isDlog = isDlog + math.log( (trainedD[x][1]+1) / (trainedD[x][2]+1) )
