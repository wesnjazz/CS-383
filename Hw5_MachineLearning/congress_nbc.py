...
def main():
	...
	totalRD = numR + numD
	pR = Decimal(Decimal(numR) / Decimal(totalRD))
	pD = Decimal(Decimal(numD) / Decimal(totalRD))
...
def predict(test_data, trainedR, trainedD, pR, pD):
	# predict_result = []
	for i in range(0, len(trainedR)):
		rr = (Decimal(trainedR[i][0])/Decimal(trainedR[i][2])) * pR
		dd = (Decimal(trainedD[i][0])/Decimal(trainedD[i][2])) * pD
